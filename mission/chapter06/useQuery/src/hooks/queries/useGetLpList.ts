...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
    return useQuery({
        ...
        // 데이터가 신선하다고 간주하는 시간.
        // 이 시간 동안은 캐시된 데이터를 그대로 사용함! 컴포넌트가 마운트되거나 창에 포커스 들어오는 경우도 재요청 x
        ...
        // retry: 쿼리 요청이 실패했을 때 자동으로 재시도할 횟수를 저장한다.
        // 기본값은 3회 정도, 네트워크 오류 등 임시적인 문제를 보완가능!

        // InitialData: 쿼리 실행 전 미리 제공할 초기 데이터를 설정합니다.
        // 컴포넌트가 렌더링 될 때 빈 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해주는 예
